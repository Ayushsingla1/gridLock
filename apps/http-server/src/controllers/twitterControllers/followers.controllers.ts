...
export const getFollowers = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const data = await axios.post(
      `https://api.apify.com/v2/acts/xbLZMZWSJDAzYUrmI/runs?token=${apifyKey}&waitForFinish=1`,
      input,
    );
    ...
    while (data2.length == 0) {
      const temp = await axios.get(
        `https://api.apify.com/v2/datasets/${dataSetId}/items?limit=50&offset=0&clean=1&view=overview`,
      );
      data2 = temp.data;
    }
    console.log(data2);
